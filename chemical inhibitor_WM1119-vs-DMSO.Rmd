...
setwd("~/RNA_seq_KAT6Ainhibitor_small_dataset")
...
```{r}
library(DESeq2)
dds<-DESeqDataSetFromMatrix(countData = countdata,colData = coldata,design = ~ Drug + Line + timept)
dds<-DESeq(dds)
plotDispEsts(dds)
```
...
#making fancy pca with labels 
#this is to plot the points on the PCA from DESeq2 with sample names on them
pcaData<-plotPCA(rld,intgroup=c("Line", "timept", "Drug"),returnData=TRUE)
percentVar<-round(100*attr(pcaData,"percentVar"))
pcaData<-merge(x=pcaData,y=coldata,by=0)
library(ggplot2)
library(ggrepel)
ggplot(pcaData, aes(PC1, PC2, color=Drug.x, shape=Line.x ,label=name)) +
  geom_point(size=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
...
morePCs<-function (object, intgroup =c("Line", "timept", "Drug"), ntop = 500, returnData = FALSE,firstPC,secondPC) 
{
...
for (i in 1:n) {
  ...
  if (gene_id %in% rownames(dds)) {
    ...
    geneCounts <- plotCounts(dds, gene = gene_id, intgroup = c("Drug", "timept", "Line"), returnData = TRUE)
